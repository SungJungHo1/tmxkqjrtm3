...
const Order: NextPage = () => {
  ...
  useEffect(() => {
    let liff_ID = "1657404178-3Y2gDADb"
        ...
        swal.fire({
          ...
          preConfirm: async (login) => {
            if (login === "fastfood1144"){
              ...
              const urls = `https://www.fastfood.p-e.kr/Add_Address?add1=${encodeURIComponent(data.add1)}&add2=${encodeURIComponent(data.add2)}&phone=${encodeURIComponent(data.Myphone)}&phone_f=${data.phone}&add_Name=${encodeURIComponent(data.add_Name)}&UserName=${encodeURIComponent(userId)}&UserId=${encodeURIComponent(userName)}&password=${encodeURIComponent(data.password)}&ImageIn=${data.addressPhoto[0] !== undefined ? 'yes' : 'no'}&friend=${true}`
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit(onSubmit)}>
      <Head>
        <title>ลงทะเบียนที่อยู่เพื่อน</title>
      </Head>
